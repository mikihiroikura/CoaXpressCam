 ...
 * @param   [out] pVersion - pointer to struct with versions information
 * @return  FGSTATUS - Error code of function execution status
 */
...
typedef struct _KYFGLib_CameraScanParameters
{
    ...
    int camera_count;            // on entry number CAMHANDLEs allocated in 'pCamHandleArray', on exit number of detected cameras
    KYBOOL bRetainOpenCameras;   // KYTRUE if scan should skip currently active links and detect only new connections.
                                 // Open camera handles will not be affected by CameraScanEx() call and will be retained at the same places of array where they were returned by previous call except for camera(s) that were closed between calls.
 ...
 * @return  FGSTATUS - Error code of function execution status
 */
KAYA_API FGSTATUS CameraScanEx(FGHANDLE handle, KYFGLib_CameraScanParameters* pScanParams);
 ...
 * @param   [in] camHandle - handle to a camera
 * @param   [out] portNumber - address of int where the COM port number will be stored
